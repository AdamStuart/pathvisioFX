...
public class XrefListController  extends TableController  {
...
//	public static final DataFormat GENE_MIME_TYPE = new DataFormat("application/x-java-serialized-gene");
	...
	@FXML private TableColumn<XRefable, String> geneTypeColumn;
	@FXML private TableColumn<XRefable, String> geneNameColumn;
	@FXML private TableColumn<XRefable, String> graphidColumn;
	@FXML private TableColumn<XRefable, String> geneIdColumn;
	@FXML private TableColumn<XRefable, String> databaseColumn;
	@FXML private TableColumn<XRefable, String> dbidColumn;
	@FXML private TableColumn<XRefable, String> dataColumn;
	@FXML private TableColumn<XRefable, String> ensemblColumn;
	@FXML private TableColumn<XRefable, String> urlColumn;
	@FXML private TableColumn<XRefable, String> termsColumn;
	@FXML private TableColumn<XRefable, String> locationColumn;
	@FXML private TableColumn<XRefable, String> logger;
	@FXML private TableColumn<XRefable, String> pvalue;
	@FXML private TableColumn<XRefable, String> fdr;
...
//	GeneListTable geneTable;
	...
	@Override public void initialize(URL location, ResourceBundle resources)
	{
		...
		geneTypeColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("type"));
		graphidColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("graphid"));
		urlColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("url"));
		typeColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("type"));
		geneNameColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("name"));
		databaseColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("database"));
		dbidColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("dbid"));
		termsColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("goFunction"));
		locationColumn.setCellValueFactory(new PropertyValueFactory<XRefable, String>("chromosome"));

		logger.setCellValueFactory(new PropertyValueFactory<XRefable, String>("logger"));
		pvalue.setCellValueFactory(new PropertyValueFactory<XRefable, String>("pvalue"));
		fdr.setCellValueFactory(new PropertyValueFactory<XRefable, String>("fdr"));
		...
		theTable.setRowFactory((a) -> {
		       return new DraggableTableRow<XRefable>(theTable, Controller.GENE_MIME_TYPE, this, geneSetRecord);
			});
...
//		graphid.setUserData("T");		edgeColumns.add(graphid);
	...
	public void loadTables(Model m) {
		XRefableSetRecord record = m.getXRec();
		allColumns = record.getAllColumns();
		Model model = m;
		dumpColumns();
		theTable.getItems().addAll(record.getXRefableSet());
		...
		if (model.getEdges() != null) 
			edgeTable.getItems().addAll(model.getEdges());
...
//		setButtonsToWindowState(false);
	...
	{ 	
		if (file == null)
		{
			...
			chooser.setTitle("Save XRefable List");
...
//    			name = StringUtil.chopExtension(name) + ".txt";
	...
	private String getTextFromTable(TableView theTable) {
		...
		for (XRefable g : (List<XRefable>)theTable.getItems())
		{
			StringBuilder lineBuff = new StringBuilder();
			for (TableColumn<XRefable, ?> col : (ObservableList<TableColumn<XRefable, ?>>)theTable.getColumns())
			{
				if (!col.isVisible()) continue;
				String txt = g.getName();		//col.getText()
				if (txt == null) txt = "";
	...
	private String getTableHeader(ObservableList<TableColumn<XRefable, ?>> columns) {
		StringBuilder buff = new StringBuilder();
		for (TableColumn<XRefable, ?> col : columns)
		{
			if (!col.isVisible()) continue;
	...
	protected void processFile(File f)
	{
		...
		if (FileUtil.isCSV(f))
			open(f);
			 ...
			 geneSetRecord = readTabularText(f);
		 else if (FileUtil.isGPML(f))
			 geneSetRecord = GPML.readGeneList(f, getSpecies());

		 if (geneSetRecord != null)
		 {
			 if (theTable.getItems().isEmpty())
				 allColumns.clear();
			 allColumns.addAll(geneSetRecord.getAllColumns());
			 resetTableColumns();
			 theTable.getItems().addAll(geneSetRecord.getGeneSet());
		 }
	}
	...
	private void doMapping(File f) {
		...
		map.remove("\tkey");
		map.remove("\tvalue");
		TableColumn<XRefable, String> oldCol = findColumnByName(theTable.getColumns(), key);
		int index = theTable.getColumns().indexOf(oldCol);
		TableColumn<XRefable, String> newCol = new TableColumn<XRefable, String>();
//		Callback<CellDataFeatures<XRefable, String>, ObservableValue<String>> a;
//		newCol.setCellValueFactory(a);
		newCol.setCellValueFactory(new Callback<CellDataFeatures<XRefable, String>, ObservableValue<String>>() {
...
//		newCol.setCellValueFactory(new PropertyValueFactory<XRefable, String>(value));

		long startMapTime = System.currentTimeMillis();
		for (XRefable gene : geneSetRecord.getGeneSet())
		{
	...
	private GeneSetRecord getGeneList() {		return geneSetRecord;	}
	...
	{		
		TableViewSelectionModel<XRefable> sel = theTable.getSelectionModel();
		int ct = geneSetRecord.getRowCount();
		for (int i=0; i<ct ; i++)
		{
			if (sel.isSelected(i))
...
//		List<XRefable> sel = FXCollections.observableArrayList();
//		List<XRefable> genes = (List<XRefable>) theTable.getSelectionModel().getSelectedItems();
//		sel.addAll(genes);
...
//		theTable.getItems().clear();
//		theTable.getItems().addAll(allGenes);
//		List<TableColumn<XRefable,?>> cols = theTable.getColumns();
...
//	}
	...
	{ 
		...
		TableColumn extant =geneSetRecord.findByText(allColumns, "All IDs");
		if (extant != null) return;
		
		GeneSetRecord rec = getGeneList();
		rec.fillIdlist();
		TableColumn<XRefable, String> mapCol = new TableColumn("All IDs");
		mapCol.setPrefWidth(300);
		mapCol.setCellValueFactory(new PropertyValueFactory<XRefable, String>("idlist"));
	...
	@FXML private void doChart()	
	{
		...
		GeneSetRecord rec = getGeneList();
		if (rec == null || !geneSetRecord.hasHeaders()) 
		{
			System.err.println("doChart failed");
			return;
		}
		String[] headers = geneSetRecord.getHeader(0).split("\t");
	...
	@FXML private void doChartVisible()	
	{
		...
		List<TableColumn<XRefable,?>> cols = theTable.getColumns();
		for (TableColumn<XRefable,?> col : cols )
		{
	...
	@FXML public void doSearch()	
	{
		...
		{
			XRefable gene =  (XRefable) theTable.getItems().get(i);
			if (gene.getName().equals(text.toUpperCase()))  //match(text.toUpperCase()))
			{
	...
	private void processExpression(String text) {
		...
		for (int i =0; i<theTable.getItems().size(); i++)
		{
			...
			if (Double.isNaN(valueA))
			{
				...
				a = gene.getValue();  //getValueByName(((TableColumn)col).getText());
			}
			else a = valueA;
			...
			if (Double.isNaN(valueB))
			{
				...
				b = gene.getValue();  //gene.getValueByName(((TableColumn)col).getText());
			}
			else b = valueB;
			...
			{
				System.out.println("expression hit: " + gene.getName());
				theTable.getSelectionModel().select(i);
			}
		}
	...
	private Double getNumericValue(String str, XRefable g)
	{
		if (StringUtil.isNumber(str)) 
			return StringUtil.toDouble(str);
		Object col = findColumnContains(str);
		if (g == null || !(col instanceof TableColumn)) return Double.NaN;
		return g.getValue();  //getValueByName(((TableColumn)col).getText());
	}
