...
public class VNode extends ResizableBox {
...
//			System.out.println(String.format("lastMouse: %.2f,  %.2f", lastMouseX, lastMouseY));
	...
	protected void handleMouseDragged(final MouseEvent event) {
		   super.handleMouseDragged(event);
			if (dragActive)
			{
//				System.out.println(String.format("prevMouseY: %.2f,  %.2f", prevMouseX, prevMouseY));
//				System.out.println(String.format("prevMouseX: %.2f,  %.2f", prevMouseX, prevMouseY));
				double dx =  prevMouseX - event.getSceneX();
				double dy = prevMouseY - event.getSceneY();
//				System.out.println(String.format("Delta: %.2f,  %.2f", dx, dy));
				prevMouseX = event.getSceneX();
				prevMouseY = event.getSceneY();
//				setTranslateX(dx);
//				setTranslateY(dy);
