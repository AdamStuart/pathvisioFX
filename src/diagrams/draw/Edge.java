...
public class Edge {
	...
	public Edge(Node start, Node end, AttributeMap attr, List<GPMLPoint> points) 
	    {
    	startNode = start;
    	endNode = end;
	...
	//------------------------------------------------------------------------------------------
    private void init(List<GPMLPoint> points)
    {
		...
		if (points != null && points.size() > 1)
		{
			if (useElbowConnection)
			{
				...
				for( int i=0; i<points.size()-1; i++)
				{
					...
					double midptX = next.getX();
					double midptY = current.getY();
					polyline.getPoints().addAll(current.getX() + current.getRelX(), current.getY() + current.getRelY(), 
							midptX, midptY							);
				}
				GPMLPoint last = points.get(points.size()-1);
				polyline.getPoints().addAll(last.getX() + last.getRelX(), last.getY() + last.getRelY());

			}
			else for (GPMLPoint pt : points)
